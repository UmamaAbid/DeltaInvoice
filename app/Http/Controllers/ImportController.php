    ...
    public function importItems(Request $request)
    {
        ...
        try{
            ...
            foreach ($data as $row) {
                ...
                $category       = trim($row[6]);
                $mrp            = trim($row[7]);
                $salePrice      = trim($row[8]);
                $discountOnSale = trim($row[9]);
                $discountType   = trim($row[10]);
                $wholesalePrice = trim($row[11]);
                $purchasePrice  = trim($row[12]);
                $taxRate        = trim($row[13]);//required
                $taxName        = trim($row[14]);//required
                $taxType        = trim($row[15]);//required
                $opeingStockQty = trim($row[16]);
                $minimumStockQty= trim($row[17]);
                $itemLocation   = trim($row[18]);
                $baseUnit       = trim($row[19]);//required
                $secondaryUnit  = trim($row[20]);
                $conversionRate = trim($row[21]);
                ...
                $itemModel = Item::create([
                    ...
                    'sale_price'                =>  (!empty($salePrice)) ? $salePrice : 0,
                    ...
                    'purchase_price'            =>  (!empty($purchasePrice)) ? $purchasePrice : 0,
                ...
                $transactionResponse = $this->itemTransactionService->recordItemTransactionEntry($itemModel, [
                    ...
                    'unit_id'                   => $baseUnitId,
                    'unit_price'                => (!empty($salePrice))? $salePrice : 0,
