...
class PurchaseOrderController extends Controller
{
    ...
    public function datatableList(Request $request){
        ...
        $data = PurchaseOrder::with('user', 'party', 'purchase')
                        ...
                        ->when(!auth()->user()->hasPermissionTo('purchase.order.can.view.other.users.purchase.orders'), function ($query) use ($request) {
        ...
        return DataTables::of($data)
                    ...
                    ->addColumn('status', function ($row) {
                        if($row->purchase){
                            return "Order Completed";//.$row->purchase->id;
                        }

                        if (is_null($row->due_date) || $row->due_date == '') {
                            return 'Due Date Empty';
                        }

                        $dueDate = Carbon::parse($row->due_date);
                        $today = Carbon::today();

                        return $dueDate->lt($today) ? 'Order Overdue' : 'Order Open';
                    })
