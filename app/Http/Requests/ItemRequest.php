...
class ItemRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        if ($this->isMethod('PUT')) {
            ...
            $rulesArray['name']          = ['required', 'string', 'max:100', Rule::unique('items')->where('name', $_POST['name'])->ignore($itemId)];
            $rulesArray['item_code']     = ['required', 'string', 'max:100', Rule::unique('items')->where('item_code', $_POST['item_code'])->ignore($itemId)];
        }else{
            $rulesArray['name']          = ['required', 'string', 'max:100', Rule::unique('items')->where('name', $_POST['name'])];
