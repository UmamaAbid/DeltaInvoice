$(function() {
    ...
    function formAdjustIfSaveOperation(response){
        ...
        $.each(response.data, function(index, item) {
            totalQuantity += parseFloat(item.quantity);
             ...
             tr += `<td class="${item.days_until_expiry > 0 ? 'text-success' : 'text-danger'}">${item.days_until_expiry}</td>`;
    ...
    function showAsPerFilterType() {
        ...
        if(filterType == 'use_date'){
            ...
            $(".to_date").hide();
            $(".from_date").hide();
            $(".days_remainig_to_expire").removeClass('d-none');
            $("input[name='days_remaining']").focus();
        }
    }
