...
@endsection
        ...
        <div class="page-wrapper">
            <div class="page-content">
                    ...
                    <div class="card-header px-4 py-3 d-flex justify-content-between">
                        ...
                        <div>
                            <h5 class="mb-0 text-uppercase">{{ __('purchase.return.return') }}</h5>
                        </div>

                        @can('purchase.return.create')
                        <!-- Button pushed to the right side -->
                        <x-anchor-tag href="{{ route('purchase.return.create') }}" text="{{ __('purchase.return.return') }}" class="btn btn-primary px-5" />
                        @endcan
                    </div>
                    <div class="card-body">
                        <div class="row g-3">
                            ...
                            <div class="col-md-3">
                                ...
                                <a tabindex="0" class="text-primary" data-bs-toggle="popover" data-bs-trigger="hover focus" data-bs-content="Filter by Purchase Return Date"><i class="fadeIn animated bx bx-info-circle"></i></a>
                        ...
                        <form class="row g-3 needs-validation" id="datatableForm" action="{{ route('purchase.return.delete') }}" enctype="multipart/form-data">
                            {{-- CSRF Protection --}}
                            @csrf
                            @method('POST')
                            <input type="hidden" id="base_url" value="{{ url('/') }}">
                            <input type="hidden" id="payment_for" value="purchase-return">
        ...
        @include("modals.payment.invoice-payment", ['payment_for' => 'purchase-return'])
...
<script src="{{ asset('custom/js/purchase/purchase-return-list.js') }}"></script>
<script src="{{ asset('custom/js/modals/payment/invoice-payment.js') }}"></script>
<script src="{{ asset('custom/js/modals/email/send.js') }}"></script>
<script src="{{ asset('custom/js/sms/sms.js') }}"></script>
@endsection
