        ...
        <div class="page-wrapper">
            <div class="page-content">
                    ...
                    <div class="card-header px-4 py-3 d-flex justify-content-between">
                        ...
                        <div>
                            <h5 class="mb-0 text-uppercase">{{ __('payment.cash_in_hand') }} : <span class="text-{{ $cashInHand>=0 ? 'success' : 'danger'}} cash-in-hand">{{ $cashInHand }}</span></h5>
                        </div>
                        <!-- Button pushed to the right side -->
                        <button role="button" class="btn btn-primary px-5 make-cash-adjustment">{{ __('payment.cash_adjust') }}</button>                        
                    </div>
                    <div class="card-body">
                        <div class="row g-3">
                            <div class="col-md-3">
                        ...
                        <form class="row g-3 needs-validation" id="datatableForm" action="{{ route('cash.transaction.delete') }}" enctype="multipart/form-data">
                            {{-- CSRF Protection --}}
                            @csrf
                            @method('POST')
...
<script src="{{ asset('custom/js/transaction/cash-list.js') }}"></script>
<script src="{{ asset('custom/js/modals/transaction/cash-in-hand.js') }}"></script>
@endsection
